app.controller("formActivityCtrl",['FileUploader','RQ','$scope','$routeParams',function(FileUploader,RQ,$scope,$routeParams){
    ...
    var subm=function(){
        cover();
        $scope.activity.content=document.getElementById("editor2").innerHTML;
        RQ.post(RQ.host + 'activity/man-'+($routeParams.id>-1?'upd':'add'), $scope.activity, function(res) {
            uncover();
            res=res.msg;
            if (res.code == 0) {
                uppic(res.data.id);
            } else if(res.code==-1){
                uppic($scope.activity.id);
            }else{
                alert(res.info);
            }
        });
            ...
            RQ.uploadfileWithParams(RQ.host+"pic/man-upload",fd,{
                ...
                if (res.code == 0) {
                    ...
                    if ($routeParams.id>-1) {
                        ...
                        window.location="#/formActivity/"+oid;
                    }
                } else {
                    alert(res.info);
                }
	...
	var updateInit = function() {
        cover();
		RQ.get(RQ.host + 'activity/man-detail', {
			"id": $routeParams.id
		}, function(res) {
            res=res.msg;
			if (res.code == 0) {
				$scope.activity=res.data.activity;
                document.getElementById("editor2").innerHTML=$scope.activity.content;
            } else {
                alert(res.info);
            }
            uncover();
    ...
    $scope.goback=function(){
        window.location="#/tableActivitys";
    }
