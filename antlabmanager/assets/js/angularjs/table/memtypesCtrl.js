app.controller("tableMemtypesCtrl",['$scope','RQ',function($scope,RQ){
    ...
    $scope.del=function(memtype,index){
        if (confirm("将要删除‘"+memtype.name+"’！请确认，删除后将无法恢复！")) {
            $scope.main.complete=false;
            RQ.get(RQ.host + 'memtype/man-del', {
                id:memtype.id
            }, function(res) {
                res=res.msg;
                if (res.code == 0) {
    ...
    $scope.init=function(){
        cover();
        RQ.get(RQ.host+"memtype/man-list",{
            pageNo:$scope.pageNo,
