app.controller("tableActivitysCtrl",['$scope','RQ',function($scope,RQ){
    ...
    $scope.del=function(activity,index){
        if (confirm("将要删除‘"+activity.title+"’！请确认，删除后将无法恢复！")) {
            $scope.main.complete=false;
            RQ.get(RQ.host + 'activity/man-del', {
                id:activity.id
            }, function(res) {
                res=res.msg;
                if (res.code == 0) {
    ...
    $scope.init=function(){
        cover();
        RQ.get(RQ.host+"activity/man-list",{
            pageNo:$scope.pageNo,
            ...
            res=res.msg;
            if (res.code==0) {
                $scope.activitys=res.data.activitys;
