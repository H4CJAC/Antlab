app.controller("tableResearchsCtrl",['$scope','RQ',function($scope,RQ){
    ...
    $scope.del=function(research,index){
        if (confirm("将要删除‘"+research.name+"’！请确认，删除后将无法恢复！")) {
            $scope.main.complete=false;
            RQ.get(RQ.host + 'research/man-del', {
                id:research.id
            }, function(res) {
                res=res.msg;
                if (res.code == 0) {
    ...
    $scope.init=function(){
        cover();
        RQ.get(RQ.host+"research/man-list",{
            pageNo:$scope.pageNo,
